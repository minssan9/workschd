<template>
  <div>
    ...
    <div v-if="stores.length === 0" class="text-center q-pa-lg text-grey-7">
      <q-icon name="store" size="48px" />
      <div class="text-h6 q-mt-sm">{{ t('team.workplace.noStores', 'No stores found') }}</div>
...
const fetchStores = async () => {
  try {
    // In a real app, fetch from API
    // const response = await fetch(`/teams/${props.teamId}/stores`)
...
const handleSubmit = async () => {
  try {
    ...
    if (dialogMode.value === 'add') {
      ...
      // await fetch(`/teams/${props.teamId}/stores`, {
      //   method: 'POST',
      ...
      // await fetch(`/stores/${editingStoreId.value}`, {
      //   method: 'PUT',
      //   headers: { 'Content-Type': 'application/json' },
      //   body: JSON.stringify(storeForm.value)
      // })
      ...
      // For demo purposes
      const index = stores.value.findIndex(s => s.id === editingStoreId.value);
      if (index !== -1) {
        const branchName = branches.value.find(b => b.id === storeForm.value.branch_id)?.name || '';
        stores.value[index] = {
          ...stores.value[index],
